    ...
    $.jqx.dataview = function () {
        ...
        this.refresh = function (fullupdate) {
            ...
            if (this.rowschangecallback != null) {
                ...
                if (countBefore != this.rows.length) this.rowschangecallback({ type: "RowsCountChanged", data: { previous: countBefore, current: this.rows.length } });
                if (diff.length > 0 || countBefore != this.rows.length) {
                    this.rowschangecallback({ type: "RowsChanged", data: { previous: countBefore, current: this.rows.length, diff: diff } });
                }
            }
        }
