(function ($)
{
    ...
    $.extend($.jqx._jqxArray.prototype, {
        ...
        _fullRefresh: function (oldvalue, value)
        {
            ...
            if (value.indexerWidth)
            {
                var newIndexerWidth = parseInt(value.indexerWidth, 10);
                that._indexerContainer[0].style.width = newIndexerWidth + 'px';
                for (var i = 0; i < that._indexers.length; i++)
                {
                    that._indexers[i].jqxNumberInput({ width: newIndexerWidth - 2 });
                }
            }

            that._getDefaultCellValue();

            var showVerticalScrollbar = that.showVerticalScrollbar,
                showHorizontalScrollbar = that.showHorizontalScrollbar;

            that._validateProperties();

            that._showVerticalScrollbar(showVerticalScrollbar);
            that._showHorizontalScrollbar(showHorizontalScrollbar);
        ...
        _addRemoveRow: function (action)
        {
            ...
            if (action === 'add' && (that.dimensions > 1 || (that.dimensions === 1 && that.columns === 1)))
            {
                ...
                for (var j = 0; j < that.columns; j++)
                {
                    ...
                    if (that.theme !== '')
                    {
                        currentNewCell.classList.add('jqx-fill-state-pressed-' + that.theme);
                    }
                    if (that._elementGap)
                    {
        ...
        _addVScrollBarHandlers: function ()
        {
            ...
            that.addHandler(vScrollbarUpButton, 'mousedown.jqxArray' + id, function ()
            {
                if (!that.disabled)
                {
                    var min = vScrollbar.jqxScrollBar('min');
                    var value = vScrollbar.jqxScrollBar('value');

                    if (value - 1 >= min)
                    {
                        vScrollbar.jqxScrollBar('setPosition', value - 1);
                    }
                }
            });
        ...
        _addHScrollBarHandlers: function ()
        {
            ...
            that.addHandler(hScrollbarLeftButton, 'mousedown.jqxArray' + id, function ()
            {
                if(!that.disabled)
                {
                    var min = hScrollbar.jqxScrollBar('min');
                    var value = hScrollbar.jqxScrollBar('value');

                    if (value - 1 >= min)
                    {
                        hScrollbar.jqxScrollBar('setPosition', value - 1);
                    }
                }
            });
