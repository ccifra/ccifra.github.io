    ...
    $.extend($.jqx._jqxDataTable.prototype, {
        ...
        databind: function (source, reason, done) {
            ...
            this.dataview.update = function (rowschanged) {
                ...
                if (that.groups && that.groups.length > 0) {
                    ...
                    // This conditional block won't get executed as the function below this expects sort function to be already defined.
                    if (!that.source.records.sort) {
                        var items = new Array();
                        // Declared data and startindex to suppress linting errors
                        var data, startindex;
                        var index = 0;
                        $.each(data, function () {
        ...
        ensurerowvisiblebykey: function (key) {
            ...
            if (item_bottom > view_bottom) {
                return this.vScrollBar.jqxScrollBar('setPosition', item_bottom - tableheight);
            }
        },
        ...
        _handleKey: function (event) {
            ...
            var home = function () {
                ...
                if ((shift || ctrl) && that.selectionMode !== 'singlerow') {
            ...
            if (rowKey != null) {
                ...
                if (this.rowinfo[rowKey]) {
                    if (this.rowinfo[rowKey].selected && (ctrl || shift)) {
                        ...
                        that._updateSelection();
                        var displayIndex = this.getrowdisplayindex(row);
                        this._raiseEvent('rowUnselect', { key: rowKey, index: displayIndex, boundIndex: index, row: row, reason: 'userSelection' });
                        return false;
                    }
                }
            ...
            if (this.rowinfo) {
                ...
                for (var obj in this.rowinfo) {
                    ...
                    if (info.selected) {
                        ...
                        this._raiseEvent('rowUnselect', { key: obj, index: displayIndex, boundIndex: index, row: info.row, reason: 'clearingSelection' });
                    }
                }
            }
            if (refresh !== false) {
...
//                    that._renderrows(false);
        ...
        unselectrowbykey: function (key, refresh) {
            ...
            var index = this.getrowindex(row);
            var displayIndex = this.getrowdisplayindex(row);
            this._raiseEvent('rowUnselect', { key: key, index: displayIndex, boundIndex: index, row: this.rowsByKey[key], reason: 'userSelection' });
        },
